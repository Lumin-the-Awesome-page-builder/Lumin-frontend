...
export default class ProjectWsModel extends WsModelUtil {
    ...
    blockComponent(component: Component) {
        this.send("block-element", {
            project_id: this.projectId,
            path: component.findTop().map(el => el.key).reverse(),
            access: this.access
        })
    }
    ...
    async patchElementProp(component: Component, action: string, prop_name: string, prop_value: any) {
    ...
    async replaceProp(component: Component, prop_name: string, prop_value: any) {
        return await this.patchElementProp(component, "replace", prop_name, prop_value);
    }

    async patchElement(component: Component) {
        return await this.sendAwaited(`patch-awaited-${this.access}`, "patch-tree", {
            project_id: this.projectId,
            access: this.access,
            path: component.findTop().map(el => el.key).reverse(),
    ...
    async patchElementOrdering(path: string[], ordering: string[]) {
        return await this.sendAwaited(`patch-ordering-awaited-${this.access}`, "patch-item-ordering", {
            project_id: this.projectId,
            access: this.access,
            path,
            ordering
    ...
    async removeElement(path: string[]) {
        return await this.sendAwaited(`remove-awaited-${this.access}`, "remove-element", {
            project_id: this.projectId,
            access: this.access,
            path,
        })
